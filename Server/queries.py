...
def addNewAdmin(Name, Username, Password, Email):
    ID = genID(admins)
    x = models.Admin(adminID = ID, name = Name, username = Username, password =
...
def addNewTrainer(Name, Address, Phone, Email, Username, Password):
    ID = genID(trainers)
    x = models.Trainer(trainerID = ID, name = Name, address = Address, phone =
...
def addNewDel(Name, Username, Password, Class, Email):
    ID = genID(delegates)
    x = models.Delegate(delID = ID, name = Name, username = Username, password =
    Password, classList = Class, email = Email)
    db.session.add(x)
    db.session.commit()
    return x
